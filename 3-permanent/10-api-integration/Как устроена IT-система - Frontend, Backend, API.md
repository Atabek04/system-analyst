---
title: "Как устроена IT-система: Frontend, Backend, API"
type: lesson
...
![Клиент → API → Сервер](assets/api-fundamentals/01-client-api-server.png)
...
> [!question] Почему цифра баланса не может храниться прямо в вашем телефоне? Подумайте 5 секунд, прежде чем читать дальше.
...
![Frontend (зал ресторана) и Backend (кухня)](assets/api-fundamentals/02-frontend-backend.png)
...
> [!tip] Frontend, это не только сайт в браузере. Мобильное приложение на телефоне, программа на компьютере, всё это тоже frontend. Объединяет их одно: с ними напрямую работает человек.
...
![Макет в Figma: экраны Register и Login нарисованы, данные вписаны вручную и не меняются](assets/api-fundamentals/04-figma-design.jpg)
...
> [!question]- Пользователь жалуется: «я ввёл скидочный промокод, а скидка не применилась». Это скорее проблема frontend (зала) или backend (кухни)? Почему?
>
...
> [!tip] Сервер, это не что-то волшебное. Это обычный мощный компьютер, который включён круглосуточно (24/7) и подключён к интернету, чтобы в любой момент принять запрос и ответить. Ваш ноутбук тоже мог бы стать сервером, если бы никогда не выключался и слушал входящие запросы.

> [!tip] По сути это два взгляда на одни и те же части. Frontend обычно выступает как клиент (просит и показывает), а Backend как сервер (обрабатывает и отвечает).
...
![Backend ЦОНа: сервер для телефона, клиент для МВД, налоговой и Минздрава](assets/api-fundamentals/03-client-server-gov.png)
...
> [!question]- Наше приложение запрашивает у банка остаток по карте. Кто здесь клиент, кто сервер, и почему backend вдруг оказался в роли клиента?
>
...
![Клиент (Consumer) шлёт Request через контракт (API Contract) к серверу (Producer), назад идёт Response](assets/api-fundamentals/07-api-contract.png)
...
> [!question]- Почему API называют «контрактом», а не просто «соединением»? Что именно он гарантирует?
>
...
![Разные протоколы передают данные по-своему: SOAP в XML, REST по ресурсам, gRPC в бинарном виде, WebSocket пушит, Webhook шлёт async](assets/api-fundamentals/05-protocols.png)
...
![Sync (звонок): оба на линии, вы ждёте ответ. Async (письмо): отправил и сразу занялся другим](assets/api-fundamentals/06-sync-async.png)
...
> [!tip] **Правило для запоминания.** Чтобы выбрать режим, спроси себя: *«Могу ли я продолжить, не дожидаясь ответа?»*
...
> [!question]- Пользователь регистрируется, и после регистрации система шлёт ему приветственное письмо. Отправку письма стоит делать синхронно или асинхронно? Обоснуйте через правило выше.
>
...
> [!question]- **1.** Почему цифра баланса не хранится прямо в телефоне? Где она живёт и почему именно там?
...
> [!question]- **2.** Объясните своими словами разницу между Frontend и Backend. Приведите по одному примеру того, за что отвечает каждый.
...
> [!question]- **3.** Какой режим, sync или async, выбрать, если после нажатия «Купить» нужно дождаться подтверждения оплаты, прежде чем показать чек? Почему?
...
> [!question]- **4.** Spot the bug. Разработчик говорит: «список товаров и их цены я зашью прямо во frontend, чтобы не дёргать сервер». Что не так?
> ```
...
> [!question]- **5.** True / False + объясните: «API, это просто провод, который соединяет frontend и backend».
...
> [!question]- **6.** Design mini-task. Наша система должна получить паспортные данные пользователя из базы МВД. Опишите в двух словах: кто здесь клиент, кто сервер, и через что они общаются?
