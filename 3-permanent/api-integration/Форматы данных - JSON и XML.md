...
> [!question]- В JSON выше поле `age` записано как `25`, а `zip` как `"050000"`. Почему возраст без кавычек, а индекс в кавычках? Что будет с ведущим нулём, если записать индекс числом?
...
![Каталог Kaspi](assets/data-formats/kaspi_catalog.png)

**2. Карточка одного товара.**

![Карточка товара](assets/data-formats/product_page.png)

**3. Отзывы этого товара.**

![Отзывы товара](assets/data-formats/product_reviews.png)
...
> [!note] Именно такие контракты (структуру запроса и ответа) Smart Bridge и хранит: по одному на каждый сервис разных министерств и ведомств.
...
> [!note] Аналогия для REST: WSDL это как **Swagger (OpenAPI)** в мире JSON.
---
...
> [!tip] Вывод для аналитика: если в задаче стоит интеграция с госсервисом РК, готовьтесь не к REST/JSON, а к SOAP + XML, контракту XSD из Smart Bridge и подписи ЭЦП на каждом запросе. Это другой мир, чем публичные REST API, и закладывать его в требования надо сразу.
...
> [!question]- **1.** Своими словами: зачем вообще нужен формат данных (JSON или XML), почему нельзя слать «просто текст»?
...
> [!question]- **2. Use-case.** Вы проектируете интеграцию с базой госоргана Казахстана через eGov. Какой формат и протокол вы закладываете в требования и почему?
...
> [!question]- **3. Spot the bug.** Разработчик присылает в SOAP-сервис вот такое поле:
> ```xml
...
> [!question]- **4. Compare.** Чем XSD отличается от WSDL? По одному предложению.
...
> [!question]- **5. True / False + объясни.** «ЭЦП в ШЭП нужна просто чтобы залогиниться, как пароль».
