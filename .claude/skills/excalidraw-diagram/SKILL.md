...
Generate `.excalidraw` JSON files that **argue visually**, not just display information — as a single
diagram, or as a multi-slide lecture deck.
...
| Mode | Output | When | Read |
|------|--------|------|------|
| **Diagram** (default) | One canvas, one visual argument | "diagram", "visualize X", "architecture of Y", "flow of Z" | This file |
| **Slide deck** | A sequence of **frames**, each a lecture slide (bullets + diagrams) | "slides", "deck", "presentation", "lecture", "lesson" | This file **+ `references/slide-decks.md`** |

A slide deck is built **on top of** this file's methodology: every diagram *inside* a slide still
obeys all the diagram rules below (isomorphism, evidence artifacts, pattern variety). Slide decks add
frames, bullet text, a typography scale, and slide types — all in `slide-decks.md`. **If the request
is for slides/a deck/a lecture, read `references/slide-decks.md` before building.**
...
**Aesthetic best-practices** (shade rules, stroke/fill, spacing, alignment, typography, the squint
test) live in `references/aesthetics.md`. Read it alongside the color palette — it applies to both
diagrams and slide decks and is what makes the output actually look beautiful, not just correct.
...
> The full aesthetic ruleset (shade/contrast rules, palette discipline, arrow style, alignment,
> typography, whitespace, the squint test) is in `references/aesthetics.md`. The essentials:
...
See `references/element-templates.md` for copy-paste JSON templates for each element type (text, line, dot, rectangle, arrow) — plus **slide-deck templates** (frame, slide title, bullet item, code block, footer) at the bottom of that file. Pull colors from `references/color-palette.md` based on each element's semantic purpose.
...
> **Render gotchas (already fixed — don't re-debug):** the renderer fetches Excalidraw from `esm.sh` at runtime, so run the render step with the **Bash sandbox disabled** (network on) or it hangs on `__moduleReady`. The import in `render_template.html` is **pinned to `@0.18.0`** — leaving it unpinned resolves to a build whose transitive deps `404`, so the module graph never loads and the wait times out. If renders suddenly start timing out, re-probe versions and re-pin rather than raising the timeout.
