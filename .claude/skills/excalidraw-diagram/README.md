...
A coding agent skill that generates beautiful and practical Excalidraw diagrams **and multi-frame lecture/presentation slide decks** from natural language descriptions. Not just boxes-and-arrows - diagrams that **argue visually**, and slides that teach.
...
- **Two modes: single diagrams and slide decks.** Build one visual argument, or a full lecture deck where each frame is a slide (bullet text + diagrams), with a typography scale, slide types, and a grid layout. See `references/slide-decks.md`.
...
- **Renderer needs network at runtime** — `render_template.html` imports Excalidraw from `esm.sh`. Run the render step with the Bash sandbox disabled, or it times out on `__moduleReady`.
- **Pin the Excalidraw version.** Unpinned `@excalidraw/excalidraw?bundle` resolves to `0.18.1`, whose `?bundle` emits transitive imports that **404** → module graph never loads → `wait_for_function` timeout. Template is pinned to `@0.18.0` (verified working). If renders start timing out after an esm.sh change, re-probe versions and re-pin here.
...
excalidraw-diagram/
  ...
  references/
    ...
    aesthetics.md                   # Beauty rules: shade, spacing, alignment, typography
    slide-decks.md                  # Lecture/presentation deck methodology (frames as slides)
    element-templates.md            # JSON templates for each element type + slide-deck templates
    json-schema.md                  # Excalidraw JSON format reference (incl. frame element)
