...
В прошлом уроке данные ехали в теле запроса, и мы отложили вопрос: а в каком именно виде они там записаны? Почему нельзя просто «текстом», и при чём тут госуслуги Казахстана?
...
Строка в кавычках, число без кавычек, `true`/`false`, `null` (значения нет), массив в `[]`, вложенный объект в `{}`. Этого набора хватает, чтобы описать почти что угодно.
...
> ❓ Одни и те же данные в JSON занимают меньше места, чем в XML. Почему тогда крупные системы (банки, госорганы) до сих пор гоняют XML, а не переписали всё на JSON?
...
> ❓ Зачем SOAP-сервису отдельный XSD-контракт, если данные и так едут в XML? Что XSD ловит такого, что «просто XML» пропустит?
...
> 💡 Вывод для аналитика: если в задаче стоит интеграция с госсервисом РК, готовьтесь не к REST/JSON, а к SOAP + XML, контракту XSD/WSDL из Smart Bridge и подписи ЭЦП на каждом запросе. Это другой мир, чем публичные REST API, и закладывать его в требования надо сразу.

> ❓ Банк отправляет в ШЭП запрос без ЭЦП, только с логином и паролем. Почему шлюзу этого мало для операции с персональными данными?
...
Новичок думает, что «данные это данные», и не различает форматы. Приходит на проект с госинтеграцией, закладывает в требования REST и JSON, а там ШЭП, SOAP и XML с обязательной ЭЦП. Переделка всей спецификации.

> ⚠️ Не делайте так: не пишите «интеграция по REST API», не проверив, как именно устроена принимающая сторона. У госсервисов РК это почти всегда SOAP/XML через ШЭП.
