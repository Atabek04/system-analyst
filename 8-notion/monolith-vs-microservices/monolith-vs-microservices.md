# 🧱 Почему систему разрезают на части
...
![Монолит: один процесс, один деплой, одна база данных](img/monolith.png)
...
> ❓ Если монолит удобнее, почему вообще кто-то от него уходит? Попробуйте назвать хотя бы один минус до того, как читать дальше.
...
![Изменили один модуль, пересобирать приходится всё приложение](img/pain-deploy.png)
...
![Чтобы выдержать нагрузку на поиск, копируется всё приложение целиком](img/pain-scaling.png)
...
> ❓ Какая из трёх болей появится раньше в стартапе из четырёх человек? А в компании из двухсот разработчиков?
...
![Три слоя: Presentation, Business Logic, Data Access](img/layered.png)
...
> ❓ Почему разделение на слои не даёт выкатить платежи отдельно от отчётов?
...
![Каждый сервис деплоится отдельно и имеет свою базу данных](img/microservices.png)
...
> ❓ Раньше модуль вызывал другой модуль внутри одной программы. Теперь между ними сеть. Что из-за этого может пойти не так?
...
> ❓ Почему проблема «сервис не ответил» не существовала в монолите? Что там происходило вместо этого?
...
![Сервисы разные, но связаны между собой так плотно, что деплоятся только вместе](img/distributed-monolith.png)
...
> ⚠️ Признак, по которому это видно сразу: в релизном чеклисте написано, в каком порядке катить сервисы.
...
> ⚠️ Важная деталь, которую теряют почти все пересказы этой истории: переписали **один компонент**. Сам Prime Video как был, так и остался микросервисным. Это точечное решение, а не отказ от микросервисов.

Обе истории про одно и то же: цена разреза оказалась выше пользы от него. Но причины разные, и это стоит различать.

> ❓ Что общего между кейсом Segment и кейсом Prime Video? Чем они отличаются?
...
![Один деплой, но между модулями жёсткие границы](img/modular-monolith.png)
...
> ❓ Модульный монолит закрывает боль 1 (независимый деплой)? А боль 3 (команды мешают друг другу)?
...
![Три команды порождают три сервиса: структура системы повторяет структуру команд](img/conway.png)
...
> ⚠️ Не делайте так: не предлагайте микросервисы как улучшение по умолчанию. Микросервисы это решение организационной проблемы, и начинать надо с вопроса, есть ли она у вас.

---

## 🧠 Module Check

> 💡 **1.** Объясните своими словами, почему слоёная архитектура не даёт независимый деплой.
...
> 💡 **2.** Стартап, 4 разработчика, продукт ещё проверяет гипотезы, CI/CD нет. Команда предлагает сразу нарезать 12 микросервисов. Что вы ответите и почему?
...
> 💡 **3.** Найдите ошибку в описании архитектуры:
> ```
...
> 💡 **4.** Чем модульный монолит отличается от микросервисов? Одно предложение на каждый.
...
> 💡 **5.** True или False: «Amazon Prime Video отказались от микросервисов и вернулись к монолиту». Обоснуйте.
...
> 💡 **6.** Вам дали задачу описать требования к системе, которую планируют резать на сервисы. Какие три вещи вы соберёте в первую очередь?
...
1. Notion → Import → Markdown/CSV → выбрать monolith-vs-microservices-notion.zip
   (именно ZIP, а не .md: картинки из img/ подтянутся только из архива,
    при импорте одного .md файла все 8 изображений отвалятся)
2. Вручную конвертировать:
   - Все "> ❓" → Callout блок (жёлтый)
   - Все "> ⚠️" → Callout блок (красный)
   - Все "> 💡" в Module Check → Toggle блок (ответ скрыт)
3. Проверить таблицы и код-блок
-->
