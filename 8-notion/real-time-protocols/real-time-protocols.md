# ⚡ Real-time: как сервер пишет первым
...
> ⚠️ Long Polling это костыль поверх HTTP, а не протокол. В новых системах его берут только когда выбора не оставили: корпоративный или государственный прокси режет WebSocket, легаси-клиент не умеет SSE, или события настолько редкие, что оверхед никого не волнует.
...
> ⚠️ Отдельно про межведомственные интеграции (ШЭП / Smart Bridge). Когда ваша система запрашивает данные у другого ведомства и ответ идёт минутами, это **не** real-time. Там работает другой механизм, **callback**: ответ приходит отдельным входящим запросом. Разбираем в уроке «Асинхронные интеграции и Callback».
...
> 💡 **6.** Если long polling доставляет данные почти мгновенно, зачем нужен SSE? Назовите два минуса long polling.
...
> 💡 **7. True / False + объясни.** «SSE и WebSocket это два real-time протокола».
